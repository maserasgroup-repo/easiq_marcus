...
`easiq-marcus` is a small Python package to analyze single-electron transfer (SET) and related Marcus-theory problems from Gaussian files.
...
## Worked example: BI(.) to Re catalyst SET
...
All of these files are included in the example folder shipped with the repository.
